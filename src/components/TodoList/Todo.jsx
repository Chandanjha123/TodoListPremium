...
// Base64 encoded beep sound (short WAV)
...
export default function Todo({ todo, toggleComplete, deleteTodo, toggleImportant }) {
  ...
  useEffect(() => {
    ...
    // Only set the timeout if the reminder is in the future
    if (reminderTime > now) {
      const timeoutDuration = reminderTime - now;

      const timeoutId = setTimeout(() => {
        // Show alert with the todo text
      ...
      // Clear timeout on component unmount or reminder change
      return () => clearTimeout(timeoutId);
    }
  }, [todo.reminder, todo.text]); // Re-run effect if reminder or text changes
